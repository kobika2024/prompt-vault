...
import {
  ...
  Briefcase, BookOpen, Zap, LayoutGrid, ChevronLeft
} from 'lucide-react'
import type { Category } from '@/types'

const categories: { key: Category | 'all'; label: string; icon: React.ReactNode; color: string }[] = [
  { key: 'all', label: 'כל הפרומפטים', icon: <LayoutGrid size={16} />, color: 'text-sky-400' },
  { key: 'coding', label: 'קוד ופיתוח', icon: <Code2 size={16} />, color: 'text-emerald-400' },
  { key: 'writing', label: 'כתיבה', icon: <PenLine size={16} />, color: 'text-violet-400' },
  { key: 'image-gen', label: 'יצירת תמונות', icon: <ImageIcon size={16} />, color: 'text-pink-400' },
  { key: 'video-gen', label: 'יצירת סרטונים', icon: <Video size={16} />, color: 'text-red-400' },
  { key: 'productivity', label: 'פרודקטיביות', icon: <Zap size={16} />, color: 'text-yellow-400' },
  { key: 'learning', label: 'למידה', icon: <BookOpen size={16} />, color: 'text-blue-400' },
  { key: 'business', label: 'עסקים', icon: <Briefcase size={16} />, color: 'text-orange-400' },
  { key: 'other', label: 'אחר', icon: <Sparkles size={16} />, color: 'text-slate-400' },
]
...
export default function Sidebar({ activeCategory = 'all', counts = {}, totalCount = 0 }: SidebarProps) {
  ...
  return (
    <aside className="w-60 min-h-screen bg-[#0a0e1a] border-l border-sky-900/30 flex flex-col">
      {/* לוגו */}
      <div className="px-5 py-6 border-b border-sky-900/30">
        <Link href="/" className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-sky-400/60 text-[10px] tracking-widest">מאת kobi76</p>
          </div>
        </Link>
      </div>
      ...
      <div className="px-5 py-3 border-b border-sky-900/20">
        <div className="flex items-center justify-between text-xs">
          <span className="text-slate-500">סה״כ שמורים</span>
          <span className="text-sky-400 font-mono font-semibold">{totalCount}</span>
        </div>
        <div className="mt-2 h-1 bg-slate-800 rounded-full overflow-hidden">
          <div
            className="h-full bg-gradient-to-l from-sky-500 to-blue-600 rounded-full transition-all duration-500"
            style={{ width: `${Math.min((totalCount / 100) * 100, 100)}%` }}
          />
        </div>
        <p className="text-slate-600 text-[10px] mt-1">עוד {100 - totalCount} לסגירת 100 הראשונים</p>
      </div>
        ...
        {categories.map(({ key, label, icon, color }) => {
          ...
          return (
            ...
            >
              ...
              {isActive && <ChevronLeft size={12} className="text-sky-400" />}
            </Link>
          )
        })}
      </nav>
